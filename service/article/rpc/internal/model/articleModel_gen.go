...
var (
	...
	cacheBuzzboxArticleArticleIdPrefix = "cache:buzzboxArticle:article:id:"
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id int64) error {
	buzzboxArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBuzzboxArticleArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, buzzboxArticleArticleIdKey)
	return err
}
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id int64) (*Article, error) {
	buzzboxArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBuzzboxArticleArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, buzzboxArticleArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	buzzboxArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBuzzboxArticleArticleIdPrefix, data.Id)
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBuzzboxArticleArticleIdPrefix, primary)
}
